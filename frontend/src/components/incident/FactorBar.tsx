 ...
 * length is the score on a 0 to 1 scale. Fixed factor order and colors (validated with dataviz),
 * 2px surface gaps, rounded outer ends. Every segment also has a labeled legend row because the
    ...
    <div>
      <div className="relative">
          ...
          <div className="flex h-full gap-[2px]" style={{ width: `${Math.min(100, candidate.score * 100)}%` }}>
            {rows.map((r, i) => (
              <span
                ...
                className={`min-w-[2px] transition-opacity duration-150 ${i === 0 ? "rounded-l-full" : ""} ${i === rows.length - 1 ? "rounded-r-full" : ""}`}
                style={{
        ...
        <div
          role="tooltip"
          aria-hidden={!tip}
          className={`num pointer-events-none absolute -top-9 left-0 z-10 whitespace-nowrap rounded-control border border-line-strong bg-surface px-2 py-1 text-[11px] text-text shadow-card transition-opacity duration-150 ${tip ? "opacity-100" : "opacity-0"}`}
        >
      ...
      <ul className="mt-3.5 space-y-2.5">
        {rows.map((r, i) => (
          <li
            key={r.key}
            className={`grid grid-cols-[8px_minmax(0,1fr)_auto] items-start gap-x-2.5 transition-opacity duration-150 ${active === null || active === i ? "opacity-100" : "opacity-50"}`}
            onPointerEnter={() => setActive(i)}
            onPointerLeave={() => setActive(null)}
          >
      ...
      <p className="mt-3 flex items-baseline justify-between border-t border-line pt-2.5 text-[12px] text-text-2">
        <span>Sum of contributions</span>
        <span className="num text-[13px] font-medium text-text">{fixed(total, 4)}</span>
      </p>
    </div>
  );
}
